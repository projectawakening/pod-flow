...
contract StringSignalHash {
    
  function calculateStringSignal(bytes memory signalBytes) public view returns (uint256 signal) {
    assembly {
  ...
  function bytes32ToStringBytes(bytes32 signalValue) public pure returns (bytes memory signalBytes) {    
    string memory signalString = Strings.toHexString(uint256(signalValue));
    signalBytes = bytes(signalString);
    return signalBytes;
  }

  function addressToStringBytes(address signalValue) public pure returns (bytes memory signalBytes) {
