...
const OUTPUT_PARAMS_FILE = path.join(__dirname, '..', 'pod-data', 'inventory_proof_params.json');
...
// --- Main Script ---
async function generateInventoryParams() {
    console.log('--- Generating Inventory Validation parameters file... ---');
    ...
    console.log('--- Inventory Validation Parameters Generation Finished ---');
}

// Run the script
generateInventoryParams().catch(error => {
    console.error('An unexpected error occurred during Inventory Validation Parameters generation:', error);
    process.exit(1);
}); 