...
async function runKeyMasterProofFlow(relativeConfigPath: string, relativeParamsPath: string) {
  logStep(`Starting Full GPC Flow for Key Master Proof`);
...
runKeyMasterProofFlow(configArg, paramsArg).catch(error => {
  console.error("\n--- UNHANDLED ERROR during Key Master proof flow ---");
  console.error(error);
  process.exit(1);
}); 