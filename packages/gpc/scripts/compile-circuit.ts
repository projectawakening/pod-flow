...
const GPC_COMPILE_CONFIG_PATH = path.resolve(__dirname, '..', 'gpc-compile-config.json');
const CIRCOMKIT_BUILD_DIR_BASE = path.resolve(__dirname, '..', 'build'); // Define base for build output (matches circomkit default)
const TMP_COMPILE_BASE_DIR = path.resolve(__dirname, '..', 'build', 'tmp_compile');
...
// Helper functions for wrapper instantiation
...
async function compileCircuit(requirementsFilePath: string): Promise<string> {
    ...
    try {
        ...
        logStep(`Moving ZKey (-pkey) to ${zkeyFileDest}`);
        try {
